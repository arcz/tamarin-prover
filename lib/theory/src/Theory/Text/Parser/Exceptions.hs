...
instance Show ParsingException where
    show (UndefinedPredicate facttag) = "undefined predicate "
                                         ...
                                         -- ++ get lName lem
                                         -- ++ "."
    show (DuplicateItem (RuleItem ru)) = "duplicate rule: " ++ render (prettyRuleName ru._oprRuleE)
    show (DuplicateItem (LemmaItem lem)) =  "duplicate lemma: " ++ lem._lName
    show (DuplicateItem (RestrictionItem rstr)) =  "duplicate restriction: " ++ rstr._rstrName
    show (DuplicateItem (TextItem _)) =  undefined
    show (DuplicateItem (ConfigBlockItem b)) =  "duplicate configuration block:" ++ show b
    show (DuplicateItem (PredicateItem pr)) =  "duplicate predicate: " ++ render (prettyFact prettyLVar pr._pFact)
